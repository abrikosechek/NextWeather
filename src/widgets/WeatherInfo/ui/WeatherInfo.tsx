...
import { ICurrentWeather, IWeatherForecast } from "@/shared/model/weather.types";
...
export const WeatherInfo = React.memo(function WeatherInfo({ city, current, forecast }: Props) {
  return (
    <>
      <main className={`${styles.main} `} >
        <CurrentWeather city={city} temp={current.main.temp_max} icon={current.weather[0].icon} />
        <WeatherForecast forecastList={forecast.list} />
        <div className={`${styles.main__group}`}>
          <Wind speed={current.wind.speed} gusts={current.wind.gust} deg={current.wind.deg} />
