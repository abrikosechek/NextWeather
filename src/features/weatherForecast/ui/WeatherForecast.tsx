...
export const WeatherForecast = React.memo(function WeatherForecast({ forecastList }: Props) {
  return (
    <div
    ...
    >
      {
        forecastList.map((item) => (
          <ForecastItem key={item.dt} date={item.dt_txt} icon={item.weather[0].icon} temp={item.main.temp} />
        ))
      }
    </div>
  )
